...
if __name__ == '__main__':
	...
	if Conf.USE_GPS==1:
		gps_data = '5'
		gps = Conf.gps.sensor(Conf.gps_q)
		gps.start()
	...
	while True:
		...
		if Conf.USE_GPS==1 and not Conf.gps_q.empty():
			while not Conf.gps_q.empty():
				gps_data = Conf.gps_q.get()
                        for item in gps_data:                                                                 
                                if item in fields:                                                                
                                        values[fields[item]] = gps_data[item]                                     
					if Conf.float_re_pattern.match(str(values[fields[item]])):
						values[fields[item]] = round(float(values[fields[item]]),2)
                                else:                                                                             
                                        values[item] = gps_data[item]                                             
		display_data(disp)
		if count == 6:
			upload_data()
